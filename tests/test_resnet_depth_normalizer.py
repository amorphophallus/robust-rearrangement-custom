...
from src.dataset.depth_stats import LEGACY_DEPTH_NORMALIZER_STATS
...
class ResnetDepthNormalizerTest(unittest.TestCase):
    ...
    def test_encoder_rejects_checkpoint_without_depth_buffers(self):
        source = _make_encoder()
        source.set_depth_normalizer_stats(
    ...
    def test_two_camera_actor_checkpoint_is_self_contained(self):
        ...
        legacy_actor = _make_rgbd_actor()
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            legacy_actor.load_state_dict(missing_stats_state)
        actual_stats = legacy_actor.get_depth_normalizer_stats()
        for camera_name, expected_stats in LEGACY_DEPTH_NORMALIZER_STATS.items():
            self.assertEqual(
                actual_stats[camera_name]["count"], expected_stats["count"]
            )
            for field in ("mean", "std", "M2"):
                self.assertAlmostEqual(
                    actual_stats[camera_name][field], expected_stats[field]
                )
        self.assertTrue(
            any("legacy RGBD checkpoint" in str(item.message) for item in caught)
        )
