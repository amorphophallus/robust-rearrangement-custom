...
### 2.1 多任务 condition 主实验（Main 3-seed）

详细数据来源、checkpoint 对应关系、逐 seed 成功数、评估输入契约和历史结果登记见 [main 3-seed experiment review](./main_3seed_experiment_review_0913.md)。本节仅保留最终论文使用的 mean ± sample standard deviation 结果。

设置：DiT policy 在 FurnitureBench 的 `one_leg`、`round_table`、`lamp` 三个任务上训练；每个 checkpoint/task 评估 36 rollout。Overall 先在每个训练 seed 内对 108 个 rollout 汇总，再在训练 seed 间计算 mean ± sample standard deviation。RGB/RGB-D 使用两个 eval seed 的逐格 min；grasp condition 使用指定的历史逐格 max，具体口径见链接的审计报告。
