...
实验论证围绕这一主张逐步展开。首先，在不额外改变零件空间分布的多任务评估中，我们比较 RGB/RGB-D 与显式 GP、skill 和语义化 point condition。排除错误的原始 RGB/RGB-D checkpoint 后，同-lineage 两 seed 对照仍显示 GP+skill、colored GP 与 skill-only 的总体成功率高于 RGB-D，且平均增益在 lamp 最大。这支持“显式空间与语义条件帮助共享策略覆盖多个长时程任务”，但不再解释为无条件策略退化到单任务。
