...
class SkillAnnotator:
    ...
    def step(
        ...
        if self.verifier is not None and guidance_point is not None:
            verify_result = self.verifier.check(
                skill=skill,
...
def _get_or_create_skill_annotator(env, env_idx: int) -> SkillAnnotator:
    ...
    # Restore persisted verifier (survives annotator resets)
    saved_verifiers = getattr(env, "_skill_annotator_verifiers", None)
    if saved_verifiers is not None and env_idx < len(saved_verifiers):
        annotators[env_idx].verifier = saved_verifiers[env_idx]
...
def get_annotation_bundle_for_env(
    ...
    # Enable / disable verifier on the cached annotator
    if enable_verify:
        if annotator.verifier is None:
            from src.eval.skill_annotation_verify import SkillAnnotationVerifier
            annotator.verifier = SkillAnnotationVerifier(tolerance_m=verify_tolerance_m)
        else:
            annotator.verifier.tolerance_m = verify_tolerance_m
        # Persist across potential annotator resets
        num_envs = max(int(getattr(env, "num_envs", 1)), 1)
        if not hasattr(env, "_skill_annotator_verifiers") or env._skill_annotator_verifiers is None:
            env._skill_annotator_verifiers = [None] * num_envs
        env._skill_annotator_verifiers[env_idx] = annotator.verifier
    else:
        annotator.verifier = None
